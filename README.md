...
- **Self-contained Colab notebook** — `colab_analysis.ipynb` reimplements the core models inline
  and runs top to bottom with no local setup, producing real PD/LGD/EL and Climate-VaR tables and
  charts directly in the notebook (plus an optional section to launch the full dashboard)
...
├── colab_analysis.ipynb    # Self-contained analysis notebook: reimplements the core models
│                            # inline, produces real PD/LGD/EL and Climate-VaR/ES tables + charts.
│                            # Optional final section launches the full dashboard via Colab tunnel.
├── requirements.txt
├── README.md
└── .gitignore
```
...
**Option 2 — Run in Google Colab (no local install)**

Click the "Open in Colab" badge above, or open
[`colab_analysis.ipynb`](colab_analysis.ipynb) directly, and run all cells.
It reimplements the scenario parser, Merton credit model, and Monte Carlo
Climate-VaR engine inline (no repo clone needed) and produces real output —
a single-firm PD/LGD/Expected Loss table across all three NGFS scenarios,
a portfolio Climate-VaR/Expected Shortfall table, and matching charts. An
optional final section also launches the full interactive Streamlit
dashboard behind a temporary `localtunnel` public URL, for anyone who wants
the sliders and editable tables instead of a static run.

No data download required for either option — all scenario parameters, firm inputs, and portfolio
holdings are either interactive (sidebar sliders + editable data table in the app) or set inline as
example data (in the notebook).
...
- **Visualization:** Plotly in the dashboard (distribution plots, sensitivity charts, loss
  histograms); Matplotlib in `colab_analysis.ipynb` (installed by the notebook itself, not
  listed in `requirements.txt`, which covers the Streamlit app only)
- **Data handling:** pandas (portfolio table, CSV export, and the executive report's Markdown
  tables via `tabulate`)
...
firm) or a flat per-sector penalty (portfolio) rather than asset-level physical hazard exposure.
Expected Loss uses the *shocked* asset value as its base, so under an extreme carbon-price shock
where $V_0$ collapses toward zero, EL can appear to fall even as PD saturates near 100% —
`colab_analysis.ipynb` surfaces this directly with the Integrated Steel Producer example; a
production model would size EL off committed exposure (EAD) rather than residual shocked value. The
